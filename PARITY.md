...
| 🏆 Ranking history (`rankDeltas`, `rankSeasonBest`) | your movement over the last day/week/month plus the best rank and earliest date it was reached, all built from snapshots this device recorded. The report cannot have either answer: eBird publishes no historical board, so the only history that exists is the one the app has been keeping since you first opened it. |
| 🎯 Celebrity Birds category (`needNearby`) | the CELEBRITY category in the unified Bird Gen feed. It answers "has an unseen notable bird drawn four independent sightings at one walkable locality within chase distance in the last 24 h", which depends on a year list the report cannot know at build time. Costs nothing: it regroups the merged wave. |
| NEW-tag fresh observation (`isFresh`) | whether an app row is recent enough to carry its NEW badge. The report's rarity window has its own registered shared rule (`notableRecent` / `section_today`); this generic row-level badge helper is app-only because the archived report renders a fixed snapshot rather than a live UI badge. |
...
| 👥 Parties (`buildParties`) | collapses birders who travelled TOGETHER into one decision, so the busy-hotspot lane counts parties rather than names — a carload of five is one choice to go somewhere wearing five names, and a shared checklist is eBird stating the group outright. App-only because it exists to gate a lane the app added: the report's `hotspot_convergence` still counts observers directly. This is a real asymmetry, not a porting gap — if the report ever regains that lane it must adopt this rule too, or the two will disagree about what "five birders" means |
| ⚖️ Independence (`INDEPENDENCE`) | F124. ONE identity function, TWO readings of it: **ATTENTION** counts names (surge — "a convoy of four is four people who cared") and **DECISION** counts parties (the busy lane — "a carload is one decision"). App-only for the same reason as `buildParties`: the report has no busy-hotspot lane to take the second reading. The asymmetry to watch is that `report.py`'s surge already takes the ATTENTION reading by counting observers directly, so the two agree TODAY by coincidence rather than by construction — if the report outlives the sunset it should import the same declaration |
| ✳️ Taxon kind (`taxonKind`) | F176. Which of eBird's four buckets a year-list row belongs in: the numbered **species** total, or **escapee** / **hybrid** / **additional taxa** below it. App-only because the report's year list is built from the eBird export, which had already removed these rows before the pipeline saw them — measured: 0 of 209 seed rows are spuh/slash/hybrid. The shape is decided from the NAME before the exotic flag, because a hybrid that is also an escapee is filed under hybrids on eBird's own list; and `exoticCategory` is read from the OBSERVATION, never per species, since one Chukar is naturalized in the arid east and another is an escapee on the west side |
| ✓ Review status (`reviewState`) | whether a reviewer has ACCEPTED a rarity, shown on both twitch lists. Measured 2026-08-23 over 1,053 live WA notable rows: 77% reviewed+valid, 23% unreviewed, and **zero** reviewed-and-rejected, because eBird withholds those from public feeds — so `obsValid=false` means "nobody has looked yet", not "thrown out". App-only for now because the report is sunsetting and this is a display fact rather than a chase computation; the fields ride free on every feed including `detail=simple`, so the report could adopt it at zero API cost if it outlives the sunset |
| 🚫 Countable taxon (`countableTaxon`) | whether a row is a species you can tick at all. eBird's taxonomy has three categories that are an identification nobody finished — **spuh** (`gull/tern sp.`), **slash** (`Western/Glaucous-winged Gull`) and **hybrid** — and none can ever be a year tick or a chase target. App-only because the report's year list is built from the export rather than harvested live, so it never carried these rows; if the report outlives the sunset it must adopt the same rule or the two will disagree about what "214 species" means. Measured against all 17,891 taxa 2026-08-24: 100% recall on all three categories, **zero** plain species lost |
...
**Since v1.0.68 the app lets you CHOOSE that radius** (Settings → *How far will
you drive for a bird?*), and the Markdown report does not: it keeps
`report.CHASE_MAX_MI`. This is a deliberate, single-value divergence. The radius
only ever **filters what has already been fetched** — it never changes the fetch
plan, which is a static golden both repos are proven equal against — so the two
repos still see identical data and only disagree about which of it is "close".
The setting's own hint says so. It also states the mechanism past 31 mi: eBird
caps `dist` on its around-me feeds at 50 km, so a larger radius is served by the
whole-**county** feeds and reaches as far as those counties do, not evenly in
every direction.
...
| 🦅 ABA Code 3+ rarities | **ABA Code 3+ rarities** | ✅ | Direct (keyless) read of the public ABA Rarities alert page; for county reports it filters to the active state and groups by species exactly like `section_state_aba_rarities`. In-app login is only a fallback. Ports `aba_rba.py` incl. `_resolve_subnational1`. Both repos print Reports · Observers · Places · Days. **One deliberate divergence:** the app also shows ABA-wide locations and states/provinces from the uncapped US+Canada species feeds; the report is state-scoped. **v1.0.25/26 — the ABA-wide stat, twice wrong.** Alert row counts measured a slice of the page's fixed 500-row budget, while the species-country endpoint returns one observation per location. The app therefore says **locations ABA-wide**, not reports. **v1.0.27 detects and states the raw 500-row cap before state filtering**, including that taxonomic ordering can remove whole species from the tail. **F304 consolidates detail into Stakeout bird without deleting evidence.** Mega remains a medium-card species index; photo, name, chevron and unclaimed row space route by exact eBird code. A transient context keyed by report slug, region, alert SID, ABA scope and load generation carries merged alert/archive rows and, only after selection, that bird's state/wide promises. The one Stakeout medium card immediately shows held ABA context, exact latest report and device history, then lazily adds finder evidence, state and ABA-wide spread, GBIF history and Wikipedia above its ordinary current-place rows. Duplicate reports merge field by field, including corrected newer report tuples, so alert observer/review values survive beside detail-feed `locId`, `sciName`, evidence and comments. Failures remain failures rather than false empty results; lazy callbacks publish only for the active generation/view/code, and Back returns to the source Mega list only while that exact view remains current. |
...
| 🛬 On passage — What’s arriving and leaving | **On passage** | ⚠️ deliberate delta | The shared base is still ~2 years of weekly (`historic/{y}/{m}/{d}`) checklists **per county**, merged into one sample per week, with the same two-week arrival/departure window and `migration.py` rules. The archived Markdown report remains county-history-only. F268 enriches this existing app section — **not a second menu tile** — with a daily current-year eBird regional first-report page and bundled state-wide GBIF timing only for species absent from county history. County history wins per species. A first report is attached as an observed fact rather than overwriting the forecast; Washington's 2026 Western Tanager on January 2 and Rufous Hummingbird on January 5 show why an early record is not automatically a passage wave. F269 adds an app-only, zero-call countdown from exact published Global Big Day, October Big Day and GBBC dates; it uses the report-local calendar day and stops after the final announced date instead of inferring another one. Sensitive rows remain presence-only. Equal-time rows use species code rather than locale-sensitive common-name ordering. Shared interpretation and precedence are guarded by `birding/tests/parity/test_first_year.py`; HTML parsing, redaction and the bundled event calendar stay app-specific. |
...
**Finder attribution (v1.0.24, moved by F304).** App-only. On a
Mega-launched Stakeout card the earliest report in the feed is credited as the
finder — but **only when coverage opened before it**. If the oldest record we
hold sits at the edge of the window, the true finder is someone earlier that we
simply cannot see, so the card says "earliest report we hold" rather than
inventing a discovery. The active bird alone lazily hydrates a missing observer
name and evidence from that checklist, with generation/view/code guards before
publication.
