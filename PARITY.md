...
| 🆕 Fresh observation (`isFresh`) | whether an app row is recent enough to carry its NEW badge. The report's rarity window has its own registered shared rule (`notableRecent` / `section_today`); this generic row-level badge helper is app-only because the archived report renders a fixed snapshot rather than a live UI badge. |
